...
class Blockinfo(Blocks):
    io_names = [ "reads_completed", "reads_merged", "sectors_read", "reading_ms", "writes_completed", "writes_merged", "writing_ms", "io_in_progress", "io_ms", "io_weighted" ]
    ...
    def get_list_io(self):
        file = self.iostat
        io = match_line("io", file, "[0-9\s]+([^0-9\s]+)[\s]+(.*)")
        return io

    def get_list_block(self):
        file = self.dfblock
        blocks = match_line("block", file, "^/dev/([^\s]+)[\s]+([0-9]+)[\s]+([0-9]+)[\s]+([0-9]+)[\s]+([0-9]+)\%[\s]+(.*)")
        return blocks

    def get_list_inode(self):
        file = self.dfinode
        inodes = match_line("inode", file, "^/dev/([^\s]+)[\s]+([0-9]+)[\s]+([0-9]+)[\s]+([0-9]+)[\s]+([0-9]+)\%[\s]+(.*)")
        return inodes
    ...
    def match_line(self, type, file, regex):
        ...
        for l in fd:
            m = re.search(regex, l)
            if m:
                block_device = get_device(m.group(1))
                if (type == "io"):
                ...
                data_table[block_device.device_name] = block_device
        fd.close()
